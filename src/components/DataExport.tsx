...
const ENVIRONMENT_CATEGORIES = {
  label: "环境数据",
  icon: Droplet,
  fields: [{ value: "fluoride_concentration", label: "氟离子浓度" }],
};
...
const TEMPERATURE_CATEGORIES = {
  ...
  icon: Thermometer,
  fields: [
    { value: "temp_body_zone1", label: "机筒温度1" },
    { value: "temp_body_zone2", label: "机筒温度2" },
    { value: "temp_body_zone3", label: "机筒温度3" },
    { value: "temp_body_zone4", label: "机筒温度4" },
    { value: "temp_flange_zone1", label: "法兰温度1" },
    { value: "temp_flange_zone2", label: "法兰温度2" },
    { value: "temp_mold_zone1", label: "模具温度1" },
    { value: "temp_mold_zone2", label: "模具温度2" },
  ],
};
...
const CURRENT_CATEGORIES = {
  ...
  icon: Zap,
  fields: [
    { value: "current_body_zone1", label: "机筒电流1" },
    { value: "current_body_zone2", label: "机筒电流2" },
    { value: "current_body_zone3", label: "机筒电流3" },
    { value: "current_body_zone4", label: "机筒电流4" },
    { value: "current_flange_zone1", label: "法兰电流1" },
    { value: "current_flange_zone2", label: "法兰电流2" },
    { value: "current_mold_zone1", label: "模具电流1" },
    { value: "current_mold_zone2", label: "模具电流2" },
  ],
};
...
const MOTOR_CATEGORIES = {
  ...
  icon: Cog,
  fields: [
    { value: "motor_screw_speed", label: "主机当前速度" },
    { value: "motor_screw_torque", label: "主机当前扭力" },
    { value: "motor_current", label: "主机当前电流" },
    { value: "motor_traction_speed", label: "牵引当前速度" },
    { value: "motor_vacuum_speed", label: "真空当前速度" },
  ],
};
...
export default function DataExport() {
  ...
  const handleSelectAllLines = (checked: boolean) => {
    if (checked && productionLines?.items) {
      setSelectedLines(
        productionLines.items.map((line) => line.name.toString())
      );
    } else {
      setSelectedLines([]);
    }
  ...
  // 按类别全选字段
  const handleSelectCategoryFields = (
    categoryType:
      | "production"
      | "temperature"
      | "current"
    ...
    if (checked) {
      setSelectedFields((prev) => [
        ...prev.filter(
          (field) => !categoryFields.some((f) => f.value === field)
        ),
  ...
  // 检查是否全选
  const isAllLinesSelected = productionLines?.items
    ? selectedLines.length === productionLines.items.length
    : false;
  ...
  const handleExport = () => {
    if (
      ...
      ((selectedLines.length === 0 || selectedFields.length === 0) &&
        selectedFields.filter((field) => field === "fluoride_concentration")
          .length === 0)
    ) {
    ...
    createExport(filters, {
      onSuccess: () =>
        toast.success("已创建新的导出文件！请在导出历史中查看状态。"),
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center space-x-2">
                ...
                />
                <Label
                  htmlFor="select-all-environment"
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <h3 className="text-sm font-medium">
                  {PRODUCTION_LINE_CATEGORY.label}
                </h3>
                <span className="text-destructive">*</span>
              </div>
              <div className="flex items-center space-x-2">
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <h3 className="text-sm font-medium">
                  {PRODUCTION_BUSINESS_CATEGORIES.label}
                </h3>
                <span className="text-destructive">*</span>
              </div>
              <div className="flex items-center space-x-2">
                <Checkbox
                  ...
                  onCheckedChange={(checked) =>
                    handleSelectCategoryFields("production", checked as boolean)
                  }
                  disabled={!isALineSelected}
                />
                <Label
                  htmlFor="select-all-production"
                  className="text-xs text-muted-foreground"
                >
                  全选
                </Label>
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center space-x-2">
                ...
                />
                <Label
                  htmlFor="select-all-temperature"
                  className="text-xs text-muted-foreground"
                >
                  全选
                </Label>
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center space-x-2">
                <Checkbox
                  ...
                  onCheckedChange={(checked) =>
                    handleSelectCategoryFields("current", checked as boolean)
                  }
                  disabled={!isALineSelected}
                />
                <Label
                  htmlFor="select-all-current"
                  className="text-xs text-muted-foreground"
                >
                  全选
                </Label>
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center space-x-2">
                <Checkbox
                  ...
                  onCheckedChange={(checked) =>
                    handleSelectCategoryFields("motor", checked as boolean)
                  }
                  disabled={!isALineSelected}
                />
                <Label
                  htmlFor="select-all-motor"
                  className="text-xs text-muted-foreground"
                >
                  全选
                </Label>
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center space-x-2">
                <Checkbox
                  ...
                  onCheckedChange={(checked) =>
                    handleSelectCategoryFields("winder", checked as boolean)
                  }
                  disabled={!isALineSelected}
                />
                <Label
                  htmlFor="select-all-winder"
                  className="text-xs text-muted-foreground"
                >
                  全选
                </Label>
          ...
          <Button
            ...
            disabled={
              ...
              selectedFields.filter(
                (field) => field === "fluoride_concentration"
              ).length === 0
            }
